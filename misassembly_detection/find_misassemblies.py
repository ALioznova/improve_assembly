...
# general
NUMBER_FOR_ESTIMATION = 10000
FREQUENCY_FOR_PRINTING_UPDATES = 100000

# begins and ends
MULT_COEFF_BEG_AND_END_READS_IN_SIGMA = 10
MAX_DIST_BEG_END_READS_PARTS_OF_FRAGMENT = 2

# insert size
DEVIATION_TLEN_IN_SIGMA = 3
MIN_NUM_OF_READS_TLEN_IN_FRAGMENT_IN_SIGMA = 3
MAX_DIST_TLEN_LEFT_PARTS_OF_FRAGMENT = 100
MAX_DIST_TLEN_RIGHT_PARTS_OF_FRAGMENT = 100

# coverage
DEVIATION_ATYPICAL_COV_IN_SIGMA = 3
MIN_LEN_ATYPICAL_COV_FRAGMENT = 300
...
def estimate_coverage(coverage):
	...
	sigma_begining_reads = numpy.std(list_of_begining_reads)
	number_of_ending_reads = numpy.mean(list_of_ending_reads)
	sigma_ending_reads = numpy.std(list_of_ending_reads)
	return ((average_coverage, sigma_coverage), (number_of_begining_reads, sigma_begining_reads, number_of_ending_reads, sigma_ending_reads))


def get_atypical_coverage_and_number_of_beg_and_end_reads(coverage, average_coverage, sigma_coverage, number_of_begining_reads, sigma_begining_reads, number_of_ending_reads, sigma_ending_reads):
	...
	max_number_of_begining_reads = number_of_begining_reads + MULT_COEFF_BEG_AND_END_READS_IN_SIGMA * sigma_begining_reads
	max_number_of_ending_reads = number_of_ending_reads + MULT_COEFF_BEG_AND_END_READS_IN_SIGMA * sigma_ending_reads
	coverage_number = 0
	sum_of_len = 0
	atypical_coverage = {}
	beg_and_end_reads = {}
		...
		for i in xrange(1, len(ref_coverage)):
			...
			for read_end in ref_coverage[i]:
				if not cur_reads.has_key(read_end):
					cur_reads[read_end] = 0
				cur_reads[read_end] += 1
			if len(ref_coverage[i]) > max_number_of_begining_reads:
			...
			if cur_reads.has_key(i):
				if cur_reads[i] > max_number_of_ending_reads:
...
def analize_atypical_cov(file_name, atypical_coverage, coverage_num):
	...
	output_file.write("======================================================\n")
	output_file.write("Fragments with low or high coverage. Average coverage is " + str(coverage_num) + ".\n")
	for (ref_name, fragments) in atypical_cov_fragments.items():
		if len(fragments) > 0:
			output_file.write("\nReference " + ref_name + "\n")
		for (start, length, aver_cov) in fragments:
			output_file.write("Start position: " + str(start) + "\tlength: " + str(length) + "\taverage coverage: " + str(aver_cov) + "\n")
	output_file.write("\n\n")
	output_file.close()


def analize_large_tlen(file_name, large_tlen, average_coverage, sigma_coverage):
	min_number_of_tlen_reads = average_coverage - MIN_NUM_OF_READS_TLEN_IN_FRAGMENT_IN_SIGMA * sigma_coverage
	tlen_fragments = {}
	for (ref_name, large_tlen_ref) in large_tlen.items():
		...
		def separate_fragments(fragment):
			...
			for pair in cur_fragment:
				...
				if begin_right <= last_right_begin + MAX_DIST_TLEN_RIGHT_PARTS_OF_FRAGMENT:
					...
					if num_of_reads >= min_number_of_tlen_reads:
		...
		for pair in large_tlen_ref:
			...
			if begin_left <= last_left_begin + MAX_DIST_TLEN_LEFT_PARTS_OF_FRAGMENT:
				...
				if len(cur_fragment) >= min_number_of_tlen_reads:
	...
	for (ref_name, fragments) in tlen_fragments.items():
		...
		for (left_beg, left_end, right_beg, right_end, num_of_reads) in fragments:
			output_file.write("Intervals: " + str(left_beg) + "-" + str(left_end) + "\t::\t" + str(right_beg) + "-" + str(right_end) + "\tnumber: " + str(num_of_reads) + "\n")
	output_file.write("\n\n")
	output_file.close()


def analize_beg_and_end_reads(file_name, beg_and_end_reads, number_of_begining_reads, sigma_begining_reads, number_of_ending_reads, sigma_ending_reads):
	max_number_of_begining_reads = number_of_begining_reads + MULT_COEFF_BEG_AND_END_READS_IN_SIGMA * sigma_begining_reads
	max_number_of_ending_reads = number_of_ending_reads + MULT_COEFF_BEG_AND_END_READS_IN_SIGMA * sigma_ending_reads
	beg_and_end_pos = {}
	for (ref_name, atypical_reads) in beg_and_end_reads.items():
		...
		for (pos, reads_type, num_of_reads) in atypical_reads:
			if pos <= last_pos + MAX_DIST_BEG_END_READS_PARTS_OF_FRAGMENT:
				...
				if (has_end and has_beg):
					fragment_len = last_pos - fragment_start + 1
					if (num >= fragment_len * (max_number_of_ending_reads + max_number_of_begining_reads)):
			...
			last_pos = pos
		if (has_end and has_beg):
			fragment_len = last_pos - fragment_start
			if (num >= fragment_len * (max_number_of_ending_reads + max_number_of_begining_reads)):
				beg_and_end_pos[ref_name].append((fragment_start, last_pos, num))
	output_file = open(file_name, 'w')
	output_file.write("======================================================\n")
	output_file.write("Positions where large number of reads ends and begins.\n")
	for (ref_name, fragments) in beg_and_end_pos.items():
		if len(fragments) > 0:
			output_file.write("\nReference " + ref_name + "\n")
		for (fragment_beg, fragment_end, num) in fragments:
			if fragment_beg != fragment_end:
				output_file.write("Positions: " + str(fragment_beg) + "-" + str(fragment_end) + "\tnumber: " + str(num) + "\n")
			else:
				output_file.write("Position:  " + str(fragment_beg) + "\tnumber: " + str(num) + "\n")
	output_file.write("\n\n")
	output_file.close()
...
if __name__ == "__main__":
	...
	((average_coverage, sigma_coverage), (number_of_begining_reads, sigma_begining_reads, number_of_ending_reads, sigma_ending_reads)) = estimate_coverage(coverage)
	(atypical_coverage, beg_and_end_reads, coverage_num) = get_atypical_coverage_and_number_of_beg_and_end_reads(coverage, average_coverage, sigma_coverage, number_of_begining_reads, sigma_begining_reads, number_of_ending_reads, sigma_ending_reads)
	...
	analize_beg_and_end_reads(os.path.join(data_name, "result.txt"), beg_and_end_reads, number_of_begining_reads, sigma_begining_reads, number_of_ending_reads, sigma_ending_reads)
	analize_large_tlen(os.path.join(data_name, "result.txt"), large_tlen, average_coverage, sigma_coverage)
